...
struct RoundEditor: View {
    ...
    init(game: Binding<Game>, previousIndex: Int? = nil) {
        ...
        if let previousIndex = previousIndex {
            self._round = .init(initialValue: InProgressRound(game.wrappedValue[previousIndex]))
        } else {
            self._round = .init(initialValue: InProgressRound(game.wrappedValue.newRound()))
        }
    }
    ...
    private struct InProgressRound {
        ...
        var statusMessage: String? {
            ...
            if scores.filter({ $0 == 0}).count == 0 && scores.count == players.count {
